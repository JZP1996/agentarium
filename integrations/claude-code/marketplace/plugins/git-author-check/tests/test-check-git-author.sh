...
OUTPUT=$(run_hook "git commit -m 'test'" "$REPO")
assert_not_contains "First mismatch does not bypass permissions" "$OUTPUT" 'permissionDecision'
...
OUTPUT=$(run_hook "git commit -m 'test'" "$REPO")
assert_contains "Subsequent mismatch denies" "$OUTPUT" '"deny"'
...
assert_not_contains "No config does not bypass permissions" "$OUTPUT" 'permissionDecision'
...
OUTPUT=$(run_hook "git add . && git commit -m 'test'" "$REPO")
assert_not_contains "chained auto-fix does not bypass permissions" "$OUTPUT" 'permissionDecision'
...
assert_not_contains "Repo A auto-fix does not bypass permissions" "$OUTPUT_A" 'permissionDecision'

# Auto-fix should ALSO fire for repo B (independent state)
OUTPUT_B=$(run_hook "git commit -m 'test'" "$REPO_B")
assert_not_contains "Repo B auto-fix does not bypass permissions" "$OUTPUT_B" 'permissionDecision'
...
assert_contains "Repo A second mismatch denies" "$OUTPUT_A2" '"deny"'
...
assert_not_contains "Malformed email does not bypass permissions" "$OUTPUT" 'permissionDecision'
...
OUTPUT=$(run_hook "git commit -m 'test'" "$REPO")
assert_not_contains "Changed email auto-fix does not bypass permissions" "$OUTPUT" 'permissionDecision'
